...
import {
  DeleteReffererComponent
} from '../dialogs/delete-refferer/delete-refferer.component';
import {
  SelectionModel
} from '@angular/cdk/collections';
...
export class ListAllComponent implements AfterViewInit {
  ...
  deleteItem(i: number, row) {
    this.index = i;
    this.id = row.id;
    const dialogRef = this.dialog.open(DeleteReffererComponent, {
      data: row,
    });
...
export class ExampleHttpDatabase {
  ...
  getRepoIssues(sort: string, order: string, page: number, size: number, query: string): Observable < ReffererList > {
    ...
    return this._httpClient.get < ReffererList > (environment.apiUrl + 'user_api/getallrefferer', {
      params
    });
  }
}
